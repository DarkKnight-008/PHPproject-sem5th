...
<div class="container mt-4">
    ...
    <table class="table table-striped" id="notesTable">
        ...
        if (!$result) {
            echo "<tr><td colspan='4' class='text-center text-danger'>Error loading notes: " . $conn->error . "</td></tr>";
        } else {
            while ($row = $result->fetch_assoc()):
            ?>
                <tr>
                    <th><?php echo $i++; ?></th>
                    <td><?php echo htmlspecialchars($row['note_title']); ?></td>
                    <td><?php echo htmlspecialchars($row['note_description']); ?></td>
                    <td>
                        <button class="btn btn-success btn-sm editBtn" data-id="<?php echo $row['sno']; ?>" data-title="<?php echo htmlspecialchars($row['note_title']); ?>" data-desc="<?php echo htmlspecialchars($row['note_description']); ?>">Edit</button>
